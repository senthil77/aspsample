...
export class ChargeDetailMainComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private modalService: NgbModal, private apiClientService: ApiClientService, 
    private store: StoreService, private notify:NotificationService) { }
  ngOnInit() {
  ...
  onSubmit() {
    ...
    this.apiClientService.postMethod<ChargeDetail>(this.editChargeDetailForm.value, 'ChargeDetail').toPromise().then((data => {
    ;

      if (data.id == chargeDetailId) {
       
        this.chargeDetailList = this.chargeDetailList.map(u => u.id !== data.id ? u : data);
